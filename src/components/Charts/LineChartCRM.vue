...
const props = defineProps(["title", "number", "amount", "styles", "chartId", "chartData", "tooltiptext", "textBelow"])
...
<template>
    <div class="card mb-3 tooltips" :style="{ backgroundColor: styles }">
        <div class="card-header">
            {{ title }}
            <v-icon icon="mdi-dots-vertical" size="large" class="dots" />
        </div>
        <div class="card-body">
            <h5 class="card-title">{{ number }} Projects</h5>
            <h5 class="card-text"> <span class="pr-0"> ₦ </span>{{ amounts }}
            </h5>
        </div>
