...
const PerformanceIndicatorsLines = ref([])
const loading = ref(false)
const disableCancel = ref(false)


const removeLine = async (dealId, id) => {
    let ps = await trmsService.deleteDealsPerformanceLinkedIndicators(dealId, id)
    ...
    PerformanceIndicatorsLines.value = await trmsService.getDealsPerformanceLinkedIndicatorsByDealId(dealId)
}
const clearModal = () => {
    particulars.value = ''
    concerns.value = ''
...
const createPerformanceIndicators = async () => {
    ...
    let ps = await trmsService.createDealsPerformanceLinkedIndicators(props.selectedDealDetails.dealId, particulars.value, concerns.value, weight.value, expectedDate.value, status.value, stores.signedInUserEmail, stores.signedInUserEmail, new Date().toJSON(), new Date().toJSON())
    if (ps == true) {
        ...
        PerformanceIndicatorsLines.value = await trmsService.getDealsPerformanceLinkedIndicatorsByDealId(props.selectedDealDetails.dealId)
...
const editLine = async (values) => {
    let ps = await trmsService.editDealsPerformanceLinkedIndicators(values.dealId, values.id, values.particulars, values.concern, values.weightPercent, values.expectedDate, values.status, values.createdBy, stores.signedInUserEmail, values.createDate, new Date().toJSON())
    if (ps == 200) {
        PerformanceIndicatorsLines.value = await trmsService.getDealsPerformanceLinkedIndicatorsByDealId(values.dealId)
...
watch(() => props.selectedDealDetails, async (first, second) => {
    if (JSON.stringify(first) !== JSON.stringify(second)) {
        ...
        if (deals != undefined) {
            PerformanceIndicatorsLines.value = await trmsService.getDealsPerformanceLinkedIndicatorsByDealId(deals.dealId)
        }
    }
});
...
<template>
    <div>
            ...
            <v-card id="style-4">
                <v-card-title>
                    <div class="d-flex justify-content-between mt-3">
                        <span class="text-h5 ml-5">Add new Performance Indicator</span>
                ...
                <v-card-text>
                    <v-container style="margin-top: -30px">
                        <div class="row g-2">
                            <div class="col-12">
                                ...
                                <textarea class="form-control" rows="5" style="resize: none;" v-model="particulars"
                            ...
                            <div :class="column">
                                ...
                                <input type="number" class="form-control" v-model="weight" :disabled="notEdit">
                            </div>
                            <div :class="column">
                    ...
                    <div class="mt-4 d-flex justify-content-end gap-2">
                        <v-btn class="rfqbtn" @click="createPerformanceIndicators()" :loading="loading" :disabled="notEdit">Create</v-btn>
        ...
        <form>
            <div class="row g-2 mb-2 pb-2" v-for="i in PerformanceIndicatorsLines" :key="i.id"
                style="border-bottom: 1px solid rgb(168, 167, 167);">
                <div class="col-12">
                    <label class="form-label">Particulars </label>
                    <textarea class="form-control" rows="3" style="resize: none;" v-model="i.particulars"
                        :disabled="notEdit"></textarea>
                </div>
                <div :class="column">
                    <label class="form-label">Concerns</label>
                ...
                <div :class="column">
                    <label class="form-label">Weight (%) </label>
                    <input type="number" class="form-control" v-model="i.weightPercent" :disabled="notEdit">
                </div>
                <div :class="column">
                    <label class="form-label">Status </label>
