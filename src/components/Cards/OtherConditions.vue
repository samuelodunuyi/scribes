...
const OtherConditionsLines = ref([])
const factors = ref('')
...
const removeLine = async (dealId, id) => {
    let ps = await trmsService.deleteDealsOtherPrecedentConditions(dealId, id)
    ...
    OtherConditionsLines.value = await trmsService.getDealsOtherPrecedentConditionsByDealId(dealId)
}
const clearModal = () => {
    factors.value = ''
    yes_no.value = ''
...
const createOtherConditions = async () => {
    loading.value = true
    disableCancel.value = true
    let ps = await trmsService.createDealsOtherPrecedentConditions(props.selectedDealDetails.dealId, factors.value, yes_no.value, concerns.value, expectedDate.value, resp_party.value, status.value, stores.signedInUserEmail, stores.signedInUserEmail, new Date().toJSON(), new Date().toJSON())
    if (ps == true) {
        toast.success('Other Precedent Conditions Added Successfully')
        setTimeout(async () => {
            emitSelectedButton(false)
            dialog.value = false
            loading.value = false
        }, 1300);
        OtherConditionsLines.value = await trmsService.getDealsOtherPrecedentConditionsByDealId(props.selectedDealDetails.dealId)
...
const editLine = async (values) => {
    let ps = await trmsService.editDealsOtherPrecedentConditions(values.dealId, values.id, values.factors, values.yesNo, values.concern, values.expectedDate, values.responsibleParty, values.status, values.createdBy, stores.signedInUserEmail, values.createdDate, new Date().toJSON())
    if (ps == 200) {
        OtherConditionsLines.value = await trmsService.getDealsOtherPrecedentConditionsByDealId(values.dealId)
...
watch(() => props.selectedDealDetails, async (first, second) => {
    if (JSON.stringify(first) !== JSON.stringify(second)) {
        ...
        if (deals != undefined) {
            OtherConditionsLines.value = await trmsService.getDealsOtherPrecedentConditionsByDealId(deals.dealId)
        }
    }
});
...
<template>
    <div>
        <v-dialog v-model="dialog" width="564" height="500" transition="dialog-right-transition"
            @click:outside="dialog = false; emitSelectedButton(false)">
            <v-card id="style-4">
                <v-card-title>
                    <div class="d-flex justify-content-between mt-3">
                        <span class="text-h5 ml-5">Add new Other Precedent Condition</span>
                ...
                <v-card-text>
                    <v-container style="margin-top: -20px">
                        <div class="row g-2">
                            <div :class="column">
                                ...
                                <textarea type="text" class="form-control" v-model="factors" :disabled="notEdit" rows="5"></textarea>
                            </div>
                            <div :class="column">
                                <label class="form-label">Resp. Party </label>
                                <textarea type="text" class="form-control" v-model="resp_party" rows="5"
                    ...
                    <div class="mt-4 d-flex justify-content-end gap-2">
                        <v-btn class="rfqbtn" @click="createOtherConditions()" :loading="loading" :disabled="notEdit">Create</v-btn>
        ...
        <form>
            <div class="row g-2 mb-2 pb-2" v-for="i in OtherConditionsLines"
                style="border-bottom: 1px solid rgb(168, 167, 167);">
                <div :class="column">
                    <label class="form-label">Factors </label>
                    <textarea type="text" class="form-control" v-model="i.factors" :disabled="notEdit" rows="3"></textarea>
                </div>
                <div :class="column">
                    <label class="form-label">Resp. Party </label>
                    <textarea type="text" class="form-control" v-model="i.responsibleParty" :disabled="notEdit" rows="3"></textarea>
                </div>
                <div :class="column">
                    <label class="form-label">Yes/No </label>
