  ...
  if (JSON.stringify(first) !== JSON.stringify(second)) {
    ...
    if (newTransact != undefined) {
      if (newTransact.leadId != '' && newTransact.leadId) {
        ...
        selectedLeadId.value = newTransact.leadId
        var allFiles = await crmService.getCRMFiles()
        files.value = allFiles.filter(o => o.linkRecordID === newTransact.leadId)
      }
    }
  }
});
...
const uploadContract = async () => {
  ...
  errorMessage.value = ""
  showUploadModal.value = false
  let ps = await crmService.createCRMFile("Lead", props.selectedLeadDetails.leadId, contractUploadUrl.value, "File Upload CRM Lead", stores.signedInUserEmail, stores.signedInUserEmail, new Date().toJSON(), new Date().toJSON())
  if (ps == true) {
    toast.success('Upload Successful')
    setTimeout(async () => {
      clearModal()
      var allFiles = await crmService.getCRMFiles()
      files.value = allFiles.filter(o => o.linkRecordID === selectedLeadId.value)
    }, 1000);
...
const downloadContract = async (fileName) => {
  ...
  link.target = "_blank";
  link.href = `https://ctpmsapi.infracredit.ng/infraUpload/DownloadCRMFiles?filename=${fileName}`
...
const deleteFile = async (id) => {
  let ps = await crmService.deleteCRMFile(id)
  if (ps == 200) {
  ...
  <div>
    ...
    <form action="">
      ...
      <div v-for="file in files"
        class="d-flex p-2 border border-2 rounded justify-content-between align-items-center mb-2" style="flex-wrap: wrap;">
        <div>
          <p class="mb-0">{{ file.fileURL }}</p>
        </div>
        <div style="display: flex; flex-direction: column; gap: 1px;">
          <v-btn class="text-none" color="blue" prepend-icon="mdi-eye-outline" variant="text"
            @click="downloadContract(file.fileURL)">
            Download
          </v-btn>
          <p style="margin-left: 40px; margin-top: -5px; font-size:small; color: red; cursor: pointer;"
            @click="deleteFile(file.id)">Delete</p>
        </div>
      </div>
    </form>
  </div>
