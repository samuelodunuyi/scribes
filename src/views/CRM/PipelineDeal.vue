...
const text = ref("allPipelineLeads")
...
const crmDetails = (id) => {
    router.push(`/crm/details/pipelineLeads/${id}`);
}
...
const pipelineDeals = computed(() => {
    ...
    pipelineDealList = pipelineDealList.sort((a, b) => {
    ...
    if (pipelineDealList.length > 0) {
        pipelineDealList.forEach((element, index, array) => {
      array[index].count = index + 1
    });

    let ps = pipelineDealList.slice(indexStart.value, indexEnd.value)
    firstValue.value = ps[0].count;
    secondValue.value = ps[ps.length - 1].count
  }

    return pipelineDealList.slice(indexStart.value, indexEnd.value)
})
...
const next = async () => {
    ...
    allPipelineLeadsData.value = allTotalLeads.filter(o => o.mandateSigned == false)
    myPipelineLeadsData.value = allPersonalLeads.filter(o => o.mandateSigned == false && o.originator == stores.signedInUserEmail || o.mandateSigned == false && o.leadTransactor == stores.signedInUserEmail || o.mandateSigned == false && o.coTransactor == stores.signedInUserEmail || o.mandateSigned == false && o.transactorLegal == stores.signedInUserEmail || o.mandateSigned == false && o.administrator == stores.signedInUserEmail)
}
    ...
    <div class="wrapper">
        ...
        <div class="d-flex justify-content-between align-center mt-3 mb-3">
            <div class="intro mt-4 ">
                <h3> Pipeline Projects (CRM)</h3>
            </div>

            <div class="toggleButton">
                <span class="add-button" @click="$router.push('/crm/newlead/pipelineLeads/new')">
                    <v-icon icon="mdi-plus"></v-icon>
                </span><span>&nbsp;</span>
                <v-btn-toggle v-model=text color='#298729' group mandatory variant="elevated">
                    <v-btn size="x-small" density="compact" value="myPipelineLeads" v-on:click="text = 'myPipelineLeads'">
                        My Pipeline Projects
                    </v-btn>
                    <v-btn size="x-small" density="compact" value="allPipelineLeads" v-on:click="text = 'allPipelineLeads'">
            ...
            <div class="custom-table table-responsive-sm">
                <div class="table-responsive">
                    <table class="table" id="table">
                        ...
                        <tbody>
                            <tr v-for="i in pipelineDeals" style="cursor: pointer;"
